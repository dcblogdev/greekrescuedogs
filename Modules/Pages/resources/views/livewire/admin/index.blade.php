...
<div>
    ...
    <div class="grid gap-4 mt-5 mb-5 sm:grid-cols-1 md:grid-cols-3">
        ...
        <div class="col-span-2">
            <x-form.input type="search" name="title" wire:model.blur="title" label="none" placeholder="Search Pages" />
        </div>
    ...
    <div class="overflow-x-scroll shadow-md">
        ...
        @foreach($this->pages() as $page)
            <tr>
                <td class="flex">{{ Str::limit($page->title, 50) }}</td>
                <td>
                    {{ $page->created_at->format('jS M Y H:i A') }}
                </td>
                <td>
                        ...
                        @if(can('delete_pages'))
                            <x-modal>
                                ...
                                <x-slot name="footer">
                                    ...
                                    <button class="btn btn-red" wire:click="deletePage('{{ $page->id }}')">Delete Page</button>
                                </x-slot>
                            </x-modal>
                        @endif
                    </div>
