...
<div>
    <div class="flex justify-between">
        ...
        <div>
            ...
            @if(can('view_blog_categories'))
                <a class="btn btn-primary" href="{{ route('admin.blog.categories.index') }}">Manage Categories</a>
            @endif

            @if(can('view_blog_authors'))
                <a class="btn btn-primary" href="{{ route('admin.blog.authors.index') }}">Manage Authors</a>
            @endif
        </div>
    ...
    <div class="overflow-x-scroll shadow-md">
        ...
        @foreach($this->posts() as $post)
            <tr>
                <td class="flex">{{ Str::limit($post->title, 50) }}</td>
