...
<div>
    <div class="flex justify-between">
        ...
        <div class="flex space-x-1">
            @if(can('add_blog_authors'))
                <livewire:blog::admin.authors.add-author/>
            @endif

            @if(can('view_blog_posts'))
                <a class="btn btn-primary" href="{{ route('admin.blog.index') }}">Manage Posts</a>
            @endif
        </div>
    ...
    <div class="mt-5 mb-5 grid sm:grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="col-span-2">
            <x-form.input type="search" name="name" wire:model="name" label="none" placeholder="Search Authors" />
        </div>
