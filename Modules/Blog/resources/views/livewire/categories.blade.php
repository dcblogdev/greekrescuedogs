        ...
        @foreach($categories as $cat)
            <li class="flex items-center col-span-1 overflow-hidden bg-indigo-800 border border-gray-200 rounded-md shadow-sm">
                <div class="flex-1 px-4 py-2 truncate">
                    <a href="{{ url('categories/'.$cat->slug) }}" class="text-sm font-medium leading-5 transition duration-150 ease-in-out">{{ $cat->title }}</a>
                    <p class="text-sm leading-5 text-white">{{ $cat->posts->count() }} Posts</p>
                </div>
            </li>
        @endforeach
    </ul>
